...
Transaction.MAX_MONEY = 18181818 * 1e8; // Unifycoin has 18181818 coins
...
Transaction.prototype.checkedSerialize = function (opts) {
  ...
  if (serializationError) {
    serializationError.message += ' - For more information please see: ' +
      'http://bitcore.io/guide/transaction.html#serialization-checks';
...
Transaction.prototype.toBufferWriter = function (writer, noWitness) {
  ...
  writer.writeUInt32LE(this.time);
  return writer;
};
...
Transaction.prototype._fromNonP2SH = function (utxo) {
  var amount = !_.isUndefined(utxo.amount) ? new Unit.fromBTC(utxo.amount).toSatoshis() : Math.round(utxo.satoshis);
  utxo.satoshis = amount;
  var clazz;
...
Transaction.prototype.toBufferEx4 = function (noWitness) {
  var writer = new BufferWriter();
  return this.toBufferWriterEx4(writer, noWitness).toBuffer();
};

Transaction.prototype.toBufferWriterEx4 = function (writer, noWitness) {
  writer.writeInt32LE(this.version);

  writer.writeVarintNum(this.inputs.length);

  _.each(this.inputs, function (input) {
    input.toBufferWriterEx4(writer);
  });
...
Transaction.prototype.uncheckedSerializeEx4 = Transaction.prototype.toString = function () {
  return this.toBufferEx4().toString('hex');
};

Transaction.prototype.toBufferEx5 = function (noWitness) {
  var writer = new BufferWriter();
  return this.toBufferWriterEx5(writer, noWitness).toBuffer();
};

Transaction.prototype.toBufferWriterEx5 = function (writer, noWitness) {
  writer.writeInt32LE(this.version);
