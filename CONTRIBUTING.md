...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Unifycore methods. For example, calling `JSON.stringify` on a Unifycore object will behave as expected and call `transaction.toJSON()` and then stringify it:
